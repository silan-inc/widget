...
function naxvarBg() {
    ...
    return 0 !== navbar.length && (box = $(".page-on-center").length > 0 ? navbar.next().find(".page-on-center .page-content") : navbar.next().find(".page .page-content"), 
...
$$(".popup-splash").on("opened", function() {
    ...
    $(".owl-carousel").length > 0 && $(".owl-carousel").owlCarousel(), $(".featured-articles-slider").length > 0 && $(".featured-articles-slider").owlCarousel({
        ...
        !/^(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/i.test(phone);
    }), $$(".goAlipay").on("click", function() {
        var fd = new FormData();
        fd.append("store_id", 1), fd.append("cash_total", 1), fd.append("value_total", 0), 
        fd.append("pay_type", "ALIPAY"), $.ajax({
            url: "https://osiris.leanapp.cn/pay/pre",
            headers: {
                AccessToken: "51kn-0o4c-0sum-mm59-tylg-q3fz"
            },
            data: fd,
            processData: !1,
            contentType: !1,
            ...
            success: function(data) {
                if (0 === data.status) {
                    _url = data.data.url, alert(_url);
                    var aliPayPlus = api.require("aliPayPlus");
                    alert(aliPayPlus), aliPayPlus.payOrder({
                        orderInfo: _url
                    }, function(ret, err) {
                        api.alert({
                            title: "支付结果",
    ...
    $$(".messagebar .link").on("click", function() {
        ...
        if (0 !== messageText.length) {
            ...
            myMessages.addMessage({
                ...
                name: name,
                // Day
                day: !conversationStarted && "Today",
                time: !conversationStarted && new Date().getHours() + ":" + new Date().getMinutes()
    ...
    localStorage.setItem("newOptions", !0)), null !== localStorage.getItem("userInfo") && 0 === JSON.parse(localStorage.getItem("userInfo")).status ? $$(".withoutLogin").hide() : $(".logo-box").on("click", function() {
        console.log("don't touch me!"), $(".logo-box").off("click");
    }), $(".chart-content").length > 0) {
